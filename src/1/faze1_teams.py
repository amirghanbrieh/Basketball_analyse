...
def get_teams_summary():
    ...
    cols = ['Franchise', 'Lg', 'From', 'To', 'Yrs', 'G', 'W', 'L', 'W/L%', 'Plyfs', 'Div', 'Conf', 'Champ']
...
def get_team_seasons(team_url):
    ...
    team_name = team_name.get_text(strip=True) if team_name else team_url.split('/')[-2]
...
def main():
    ...
    summary_df.to_csv('nba_teams_summary.csv', index=False, encoding='utf-8-sig')
    ...
    if all_seasons:
        ...
        combined.to_csv('nba_teams_seasons.csv', index=False, encoding='utf-8-sig')
